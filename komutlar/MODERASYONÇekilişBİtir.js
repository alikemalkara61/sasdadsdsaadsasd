...
exports.run = async (client, message, args) => {
  ...
  if (!cekilisid)
    return message.reply(
      new Discord.MessageEmbed().setDescription(
        "Bir Çekiliş ID'si Gİrmen Lazım "
      )
    );

  if (!db.has(`cekilisidsi_${cekilisid}`))
    return message.reply("Doğru bir çekiliş IDsi belirtmeniz gerek.");

  client.channels.cache
    .get(db.fetch(`cekilisidsi_${cekilisid}.mesaj.kanal`))
    .messages.fetch(db.fetch(`cekilisidsi_${cekilisid}.mesaj.id`))
    .then(async msg => {
      if (!db.has(`cekilis_${msg.id}`))
        return message.reply(
          new Discord.MessageEmbed().setDescription("BU Çekiliş Zaten Bitmiş")
        );
